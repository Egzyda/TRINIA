...
import { configureRules, rulesForMode, type MatchModeId, type RuleSet } from '../src/core/rules';
...
function parseArgs(argv: string[]): Args {
  const args: Args = { games: 200, ai: 'normal', seed: 31337, maxHp: 10, step: 2, mode: 'standard' };
...
function main(): void {
  ...
  const rules: RuleSet = { ...rulesForMode(args.mode), MAX_BID: args.maxHp + 2 };
  ...
  console.log(
    `\n=== 先攻権の価値測定 (AI: ${args.ai} / 各条件 ${args.games} 戦 / モード=${args.mode} 拠点HP${rules.BASE_HP}) ===`,
  );
  ...
  for (let paid = 0; paid <= args.maxHp; paid += args.step) {
    ...
    for (const deck of DECK_PRESETS) {
      ...
      for (let g = 0; g < args.games; g++) {
        ...
        const winner = playFixed(deck.id, deck.id, first, paid, args.ai, seed, rules);
        if (winner !== null) {
          games += 1;
